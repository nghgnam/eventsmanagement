...
export class UserInformationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const authInstance = this.authService.getBrowserAuth();
    if (!authInstance) {
      this.isLoading.set(false);
      this.errorMessage = 'Authentication is not ready yet. Please try again.';
      return;
    }
  ...
  activeOrganizer(){
    ...
    const authInstance = this.authService.getBrowserAuth();
    if (!authInstance) {
      this.errorMessage = 'Authentication is not available right now. Please try again.';
      return;
    }
  ...
  async updatePasswordHandler(): Promise<void> {
    ...
    const authInstance = this.authService.getBrowserAuth();
    if (!authInstance) {
      this.errorMessage = 'Authentication is not available right now.';
      return;
    }
    const user = authInstance.currentUser;

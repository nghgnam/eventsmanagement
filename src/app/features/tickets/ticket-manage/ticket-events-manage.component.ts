...
import { TabContentTicketsComponent } from '../ticket-tabs/tab-content-tickets.component';
...
export class TicketEventsManageComponent implements OnInit, OnDestroy {
    ...
    const ticketsSub = this.ticketService.getAllTicketsByUserId(this.currentUser.uid).pipe(
      tap(ticketData => {
        if (ticketData.length > 0) {
          ...
          this.listTicketsEventId = this.extractEventIds(this.ticket);
        }
      }),
      switchMap(() => {
        return forkJoin({
  ...
  private processTicketData(data: TicketData): void {
    ...
    this.listUpcomingTickets = this.extractEventIds(activeTickets);
    ...
    this.listUnpaidTicketValid = this.extractEventIds(unpaidTicketsValid);
    ...
    this.listUnpaidTicketExpired = this.extractEventIds(unpaidTicketsExpired);
    ...
    this.listTicketExpired = this.extractEventIds(expiredTickets);
  ...
  private extractEventIds(tickets: TicketType[]): string[] {
    return tickets

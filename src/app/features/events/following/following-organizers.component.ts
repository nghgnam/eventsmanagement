...
type FollowWithOrganizer = Follows;
              ...
              @if (follow.organizer) {
                <div class="organizer-info">
                  <div class="organizer-avatar-container">
                    @if (getSafeImageUrl(follow.organizer.profileImage ?? undefined); as safeUrl) {
                      <img 
                        [src]="safeUrl" 
                  ...
                  <div class="organizer-details">
                    <h3>{{getOrganizerName(follow)}}</h3>
                    <p class="company-name">{{getOrganizerCompany(follow)}}</p>
                    <div class="follow-info">
                      <span class="status" [class.active]="getFollowStatus(follow) === 'active'">
                        {{getFollowStatus(follow)}}
                      </span>
                      @if (getFollowedDate(follow); as followDate) {
                        <span class="follow-date">
                          Followed since: {{followDate | date:'mediumDate'}}
                        </span>
                      }
                    </div>
                  </div>
...
export class FollowingOrganizersComponent implements OnInit, OnDestroy {
  ...
  defaultAvatar = 'https://res.cloudinary.com/dpiqldk0y/image/upload/v1744575077/default-avatar_br3ffh.png';
  ...
  getOrganizerName(follow: FollowWithOrganizer): string {
    return follow.organizer?.fullName ?? 'Unknown Organizer';
  }

  getOrganizerCompany(follow: FollowWithOrganizer): string {
    return follow.organizer?.organization?.companyName ?? 'No company name';
  }

  getFollowStatus(follow: FollowWithOrganizer): string {
    return follow.relationship?.status ?? 'inactive';
  }

  getFollowedDate(follow: FollowWithOrganizer): Date | null {
    return this.toDate(follow.timestamps?.followedAt);
  }
